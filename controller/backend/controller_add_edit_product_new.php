<?php 
	class controller_add_edit_product_new{
		public $model;
		public function __construct(){
			...
			switch($act){
				case "edit":
					$form_action = "admin.php?controller=add_edit_product_new&act=do_edit&id=$id";
					//lay mot ban ghi
					$arr = $this->model->get_a_record("select * from tbl_product_new where pk_product_id=$id");
					//load view
				...
				case "do_edit":
					...
					//edit ban ghi
					$this->model->execute("update tbl_product_new set c_name='$c_name',c_description='$c_description',c_content='$c_content',c_hotproduct=$c_hotproduct,fk_category_product_id=$fk_category_product_id where pk_product_id=$id");					
					//upload image
					$c_img = "";
				...
				case "do_add":
					...
					$this->model->execute("insert into tbl_product_new(c_name,c_description,c_content,c_img,c_hotproduct) values('$c_name','$c_description','$c_content','$c_img',$c_hotproduct)");
					//------------
					//quay lai trang tin tuc
					header("location:admin.php?controller=product_new");
				break;
	...
	new controller_add_edit_product_new();
 ?>